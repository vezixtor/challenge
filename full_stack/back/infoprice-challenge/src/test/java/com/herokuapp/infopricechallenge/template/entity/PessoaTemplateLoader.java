...
public class PessoaTemplateLoader implements CommonTemplateLoader {

    @Override
    public void load() {
        Fixture.of(Pessoa.class).addTemplate(RULE_VALID, new Rule(){{
            add("id", uniqueRandom(LONG_ONE, Long.MAX_VALUE));
            add("nome", random(RANDOM_STRINGS));
            add("email", random(RANDOM_STRINGS));
            add("enderecos", has(3).of(Endereco.class, RULE_VALID));
